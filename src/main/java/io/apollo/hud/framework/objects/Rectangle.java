...
package io.apollo.hud.framework.objects;
...
public class Rectangle extends GuiElement{
    ...
    public Rectangle size (int width, int height) {
        if (this.x2 != -1) { Apollo.error("[framework-" + this.name + "] Object controlled by points, size has been ignored!"); return this; }
        this.x2 = x1 + width;
        this.y2 = y1 + height;
        return this;
    }
    ...
    @Override public Rectangle build() {
        if (this.color == null) { this.color = Color.BLACK;
            Apollo.error("[framework-" + this.name + "] Rectangle had no color and has been defaulted to BLACK!");
            ...
            Apollo.error("[framework-" + this.name + "] Rectangle had no outline color and has been defaulted to DARK-GRAY!");
        } if (this.x2 == -1) { this.x2 = this.x1 + 10; this.y2 = this.y1 + 10;
            Apollo.error("[framework-" + this.name + "] Rectangle has no size or secondary position and has be defaulted to 10:10");
            ...
            Apollo.error("[framework-" + this.name + "] Rectangle corner was to large and have been reduced!");
    ...
    @Override public void draw (io.apollo.hud.framework.Panel panel) {
