...
public class EventBus {
  private final HashMap<Class<? extends Event>, CopyOnWriteArrayList<EventListener>> listeners =
      new HashMap<>();

  /**
   * Registers an object to the {@link EventBus}. All methods annotated with the {@link
   * EventSubscriber} annotation will be called when that Event is posted.
   *
                ...
                for (Method method : m.getDeclaredMethods()) {
                  ...
                  if (method.getParameterCount() == 0)
                    throw new IllegalArgumentException(
                        method.getName() + " doesn't have any parameters!");
                  ...
                  try {
                    ...
                    throw new IllegalArgumentException(
                        method.getName() + "'s first parameter is not an event.");
                  }
                  ...
                  listeners
                      .get(event)
                      .sort(
                          Comparator.comparingInt(listener -> listener.getEventPriority().getId()));
                }
              });
    }
  }
  ...
  public void unregister(Class<?> aClass) {
    listeners
        .values()
        .forEach(it -> it.removeIf(listener -> listener.getInstance().getClass() == aClass));
  }

  /**
   * Sets the priority of all methods with the specified event in <b>all instances</b> of the
   * specified object.
   *
  ...
  public void setPriorityOf(Object any, Event event, Priority newPriority) {
    ...
    listeners
        .getOrDefault(eventClass, new CopyOnWriteArrayList<>())
        .forEach(
            listener -> {
  ...
  void sort(Class<? extends Event> eventClass) {
    listeners
        .get(eventClass)
        .sort(Comparator.comparingInt(listener -> listener.getEventPriority().getId()));
  }
}
