...
public class DiscordRPModule extends Module {
  ...
  public void setup() {
                  ...
                  Minecraft.getMinecraft()
                      .addScheduledTask(
                          () -> {
                            ...
                            } // Returns null when they're not in a world.
                            Minecraft.getMinecraft()
                                .displayGuiScreen(
                                    new GuiConnecting(
                                        new GuiMainMenu(),
                                        Minecraft.getMinecraft(),
                                        new ServerData(serverIp, serverIp, false)));
                          });
                })
            .setJoinRequestEventHandler(
                request -> DiscordRPC.discordRespond(request.userId, DiscordRPC.DiscordReply.NO))
            .build();
    onEnabled();
    new Thread("Apollo DiscordRPC") {
      ...
      public void run() {
        while (true) {
          if (isEnabled()) DiscordRPC.discordRunCallbacks();
        }
      }
    }.start();
  }
  ...
  public void onEnabled() {
    ...
    if (discordRichPresence != null) DiscordRPC.discordUpdatePresence(discordRichPresence);
  }
  ...
  public static void update(String title, String description, String bigLogo) {
    DiscordRichPresence discordRichPresence =
        new DiscordRichPresence.Builder(description).setStartTimestamps(currentTime).build();
  ...
  public static void update(
      String title, String description, String bigLogo, String smallLogo, String smallLogoText) {
    DiscordRichPresence discordRichPresence =
        new DiscordRichPresence.Builder(description).setStartTimestamps(currentTime).build();
    discordRichPresence.writeField("details", title);
    discordRichPresence.writeField("largeImageKey", bigLogo);
